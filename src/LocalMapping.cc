...
void LocalMapping::InsertKeyFrame(KeyFrame *pKF) {
	...
	if(mlNewKeyFrames.size() > 1) std::cout << "Mapping too slow " << std::endl ;
	mbAbortBA = true;
	/*std::thread * map_thread = new std::thread(&LocalMapping::RunOnce, this);
	(*map_thread).detach();*/ //This could be used to run the mapping only when a new frame is inserted.
}
