...
static size_t write_cb(char *buf, size_t n, size_t nmemb, void *p)
{
	...
	if (sub) {
	    ...
	    received_length = len-(pos_n-buf+1);
	    memcpy(raw_jpeg_buf, pos_n+1, received_length);
	    stat = RECV_DATA;
	}
	break;
    }
	...
	if (received_length + len < content_length) {
	    ...
	    memcpy(read_buf, buf+content_length-received_length, len-(content_length-received_length));
	    read_buf_isempty = false;
	    read_buf_len = len-(content_length-received_length);
	    stat = IDLE;
